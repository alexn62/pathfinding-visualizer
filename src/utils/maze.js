...
function findNeighborClosestToStart (current, targetDistance, storage, size) {
  ...
  //left
  let leftNeighbor =
    column === 0 ? null : [[row, column - 1], storage[[row, column - 1]]];

  // top
  let topNeighbor =
    row === 0 ? null : [[row - 1, column], storage[[row - 1, column]]];
  // right
  let rightNeighbor =
    column === size - 1
      ? null
  ...
  // bottom
  let bottomNeighbor =
    row === size - 1 ? null : [[row + 1, column], storage[[row + 1, column]]];
...
function explore (maze, storage, targetPosition, queue) {
  ...
  for (let neighbor of unvisitedNeighbors) {
    ...
    storage[neighbor[0]].distance = Math.min(
...
function findUnvisitedNeighbors (size,storage, row, column) {
  ...
  leftNeighbor =
    column === 0 ||
    storage[[row, column - 1]].visited ||
    storage[[row, column - 1]].wall
      ? null
      : [[row, column - 1], storage[[row, column - 1]]];
  // top
  topNeighbor =
    row === 0 ||
    storage[[row - 1, column]].visited ||
    storage[[row - 1, column]].wall
      ? null
